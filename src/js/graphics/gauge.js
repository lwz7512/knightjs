...
import { between, interpolate } from '../math.js'
import { globals as g } from '../globals.js'

export const healthGradient = createCanvas(400, 1, (ctx) => {
...
export const staminaGradient = createCanvas(400, 1, (ctx) => {
  const grad = ctx.createLinearGradient(-200, 0, 200, 0)
...
export default class Gauge {
  ...
  render(width, height, color, half, ridgeCount) {
    const { ctx } = g
    function renderGauge(width, height, value, color) {
      ctx.wrap(() => {
    ...
    ctx.wrap(() => {
      ...
      renderGauge(
        width,
        height,
        Math.min(this.displayedValue, this.getValue()),
        color,
      )
