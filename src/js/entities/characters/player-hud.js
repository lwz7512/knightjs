...
import KingEnemy from './king-enemy.js'

export default class PlayerHUD extends Entity {
  constructor(player) {
    super()
    this.player = player
    ...
    // console.log(`### create dummy player...`)
  ...
  doRender(camera) {
    ...
    const { ctx } = g
    const { CANVAS_WIDTH, CANVAS_HEIGHT, WAVE_COUNT } = consts
    ctx.wrap(() => {
    ...
    ctx.wrap(() => {
      ...
      ctx.wrap(() => {
        ctx.translate(
          interpolate(-300, 300, this.progressGauge.displayedValue),
          20,
        )
        ctx.scale(0.5, 0.5)
    ...
    ctx.wrap(() => {
      ...
      ctx.strokeText(
        nomangle('SCORE: ') + this.player.score.toLocaleString(),
        0,
        0,
      )
      ctx.fillText(
    ...
    if (this.player.combo > 0) {
      ctx.wrap(() => {
        ...
        const ratio = Math.min(
          1,
          (this.player.age - this.player.lastComboChange) / 0.1,
        )
