...
import { isBetween, between, TWO_PI } from '../../math.js'
...
export default class Tree extends Obstacle {
  ...
  cycle(elapsed) {
    ...
    if (!this.noRegen)
      regenEntity(
        this,
        consts.CANVAS_WIDTH / 2 + 200,
        consts.CANVAS_HEIGHT / 2 + 400,
      )
  ...
  doRender() {
    ...
    ctx.withShadow(() => {
      ...
      ctx.wrap(() => {
        ctx.rotate(
          Math.sin(
            ((this.age + this.rng.next(0, 10)) * TWO_PI) / this.rng.next(4, 16),
          ) * this.rng.next(Math.PI / 32, Math.PI / 64),
        )
        ...
        for (let i = 0; i < 5; i++) {
          ...
          ctx.wrap(() => {
            ...
            ctx.rotate(
              (Math.sin(
                ((this.age + this.rng.next(0, 10)) * TWO_PI) /
                  this.rng.next(2, 8),
              ) *
                Math.PI) /
                32,
      ...
      if (!ctx.isShadow) {
        for (const character of this.scene.category('enemy')) {
          if (
            ...
            ctx.resolveColor = () =>
              character instanceof Player ? '#888' : '#400'
            ctx.wrap(() => {
