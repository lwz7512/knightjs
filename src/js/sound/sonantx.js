...
// Oscillators
export const osc_sin = (value) => {
  return Math.sin(value * 6.283184)
}

export const osc_square = (value) => {
  return osc_sin(value) < 0 ? -1 : 1
}

export const osc_saw = (value) => {
  return (value % 1) - 0.5
}

export const osc_tri = (value) => {
  const v2 = (value % 1) * 4
  return v2 < 2 ? v2 - 1 : 3 - v2
}

// Array of oscillator functions
export const oscillators = [osc_sin, osc_square, osc_saw, osc_tri]

export const getnotefreq = (n) => {
  return 0.00390625 * Math.pow(1.059463094, n - 128)
}

export const genBuffer = (waveSize, callBack) => {
  setTimeout(() => {
...
export const applyDelay = (chnBuf, waveSamples, instr, rowLen, callBack) => {
...
export class AudioGenerator {
  ...
  getAudioBuffer(callBack) {
    ...
    const buffer = g.audioCtx.createBuffer(WAVE_CHAN, this.waveSize, WAVE_SPS) // Create Mono Source Buffer from Raw Binary
...
export class SoundGenerator {
  ...
  genSound(n, chnBuf, currentpos) {
    ...
    for (var j = this.attack + this.sustain + this.release - 1; j >= 0; --j) {
      ...
      // Envelope
      let e = 1
      if (j < this.attack) e = j / this.attack
...
export class MusicGenerator {
  ...
  // FIXME: unnecessary complexity doing this ...
